...
import { ChevronLeft, ChevronRight, ChevronDown, Truck, Tag, Cpu, Server, HardDrive, Fan, Plug, MemoryStick, CircuitBoard, Layers } from "lucide-react";
...
export default function PreBuiltPCPageClient({ product, relatedProducts }: PreBuiltPCPageClientProps) {
  ...
  const [openExpertItems, setOpenExpertItems] = useState<Set<number>>(new Set());
  const [openWhyItems, setOpenWhyItems] = useState<Set<number>>(new Set());

  const toggleExpertItem = (idx: number) => {
    setOpenExpertItems((prev) => {
  ...
  return (
    <div className={styles.productPageWrapper}>
      ...
      <section className={styles.productGrid}>
        ...
        <section className={styles.specsGridSection}>
        ...
        {/* Expert Assessment Section - Collapsible */}
        {(() => {
          const expertFields = [
          ...
          return (
            <section className={styles.collapsibleSection}>
              <div className={styles.collapsibleSectionHeader}>EXPERT ASSESSMENT</div>
              <div className={styles.collapsibleList}>
                {items.map((item, idx) => (
                  <div key={idx} className={styles.collapsibleItem}>
                    <button
                      className={`${styles.collapsibleToggle} ${openExpertItems.has(idx) ? styles.collapsibleToggleOpen : ""}`}
                      onClick={() => toggleExpertItem(idx)}
                      aria-expanded={openExpertItems.has(idx)}
                    >
                      <span className={styles.collapsibleTitle}>{item.title}</span>
                      <ChevronDown size={18} className={styles.collapsibleChevron} />
                    </button>
                    {openExpertItems.has(idx) && (
                      <div
        ...
        {(() => {
          ...
          return (
            <section className={styles.collapsibleSection}>
              <div className={styles.collapsibleSectionHeader}>WHY WE CHOSE THESE COMPONENTS?</div>
              <div className={styles.collapsibleList}>
                {items.map((item, idx) => (
                  <div key={idx} className={styles.collapsibleItem}>
                    <button
                      className={`${styles.collapsibleToggle} ${openWhyItems.has(idx) ? styles.collapsibleToggleOpen : ""}`}
                      onClick={() => toggleWhyItem(idx)}
                      aria-expanded={openWhyItems.has(idx)}
                    >
                      <span className={styles.collapsibleTitle}>{item.title}?</span>
                      <ChevronDown size={18} className={styles.collapsibleChevron} />
                    </button>
                    {openWhyItems.has(idx) && (
                      <div
                        className={styles.collapsibleContent}
                        dangerouslySetInnerHTML={{ __html: String(item.value) }}
                      />
