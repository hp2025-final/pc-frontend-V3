...
export async function GET() {
  ...
  try {
    console.log("[Sitemap Products 2] Fetching products 1001-2000...");
    
    // Fetch products 1001-2000 (pages 11-20)
    ...
    const products = batches.flat().filter(p => p && p.slug); // Filter out empty results
    
    console.log(`[Sitemap Products 2] Fetched ${products.length} products`);
...
${productRoutes
  ...
  .join("\n")}
    ...
    console.error("Error generating products-2 sitemap:", error);
